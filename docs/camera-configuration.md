...
| `tracking_down-ar0144.json` | `tracking_down` | 1 | The AR0144 tracking camera |
...
The `tracking_down` entry is what produces the stream the precision-landing
stack consumes:

- **`en_misp` / `misp_*`** - the MISP path produces the encoded output at
  1280x800. `misp_venc_enable: true` is what creates the
  `tracking_down_misp_encoded` pipe used for streaming.
